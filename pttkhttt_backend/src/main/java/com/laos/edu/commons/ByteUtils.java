...
public class ByteUtils {
    ...
    public static String bytesToASCIIString(byte b[]) {
        ...
        for (int i = 0; i < b.length; i++) sb.append((char) ((char) b[i] & 0xff));
    ...
    public static byte[] stringToBytes(String s, int minLength) {
        if (s.length() % 2 != 0) s = "0".concat(s);
        int len = s.length() / 2;
        int padLength = len;
        if (padLength < minLength) padLength = minLength;
    ...
    public static String bytesToString(byte b[]) {
        ...
        for (int i = 0; i < b.length; i++) sb.append(bch[b[i] & 0xff]);
    ...
    public static byte[] ensureDESParity(byte desKey[]) {
        for (int i = 0; i < desKey.length; i++) desKey[i] = setOddParity(desKey[i]);
    ...
    public static byte setOddParity(byte b) {
        b &= 0xfe;
        int setBits = 0;
        for (int i = 0; i < 8; i++) if ((b >> i & 1) != 0) setBits++;
    ...
    public static void bytesToFile(byte b[], String filename) {
        try {
            ...
            int bytesRead;
            while ((bytesRead = bis.read(buffer)) != -1) fos.write(buffer, 0, bytesRead);
        } catch (IOException e) {}
    }
    ...
    public static String bytesToDebugString(byte b[]) {
        ...
        if (b != null && b.length > 0) {
            ...
            for (int i = 1; i < b.length; i++) sb.append((new StringBuilder()).append(" 0x").append(bch[b[i] & 0xff]).toString());
        }
        return sb.toString();
    }
    ...
    public static String bytesToDebugString(byte b[], int length) {
        ...
        if (b != null && b.length > 0 && length > 0) {
            ...
            sb.append((new StringBuilder()).append("0x").append(bch[b[0] & 0xff]).toString());
            for (int i = 1; i < length; i++) sb.append((new StringBuilder()).append(" 0x").append(bch[b[i] & 0xff]).toString());
        }
        return sb.toString();
    }
